...
let access_some_aux base access key =
  ...
  | [ ip ] ->
      ...
      if get_access p <> access then
        let p = { (gen_person_of_person p) with Def.access } in
...
let () =
  ...
  let speclist =
    [
      ...
      ( "--target",
        Arg.String (fun s -> set_target_access_of_string s),
        "[public|default|private] Proccess only persons with given access" );
      ("--everybody", Arg.Set everybody, " process the wole base.");
      ("--key", Arg.String (fun s -> list := s :: !list), "<KEY>");
      ( "--keys",
        Arg.String (fun s -> input_file list s),
        "<FILE> file containing one key per line." );
    ]
    |> Arg.align
  in
  let anonfun i = bname := i in
  let usage =
    "Usage: " ^ Sys.argv.(0) ^ " [-everybody] [-ind key] [-list-ind file] base"
  in
