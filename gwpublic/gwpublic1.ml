...
*)
let compute_ndgen treshold y = (treshold - y) * nb_gen_by_century / 100
...
let mark_descendants base scanned old treshold =
  let rec loop p ndgen =
    ...
    if Gwdb.Marker.get scanned p_key_index < ndgen then
      (* If we did not already scanned with ndgen >= current ndgen *)
      let ndgen =
        match Gwaccess_util.most_recent_year_of p with
        | Some y ->
      ...
      if ndgen > 0 then (
        ...
        Array.iter
          (fun ifam ->
            ...
            if Gwdb.Marker.get scanned sp < ndgen then
              let ndgen'' =
                Option.fold ~none:ndgen ~some:(compute_ndgen treshold)
              ...
              if ndgen'' > 0 then (
                ...
                Array.iter
                  (fun c ->
                    loop (poi base c) (min ndgen' (max 0 (ndgen'' - 1))))
                  (get_children fam)))
          (get_family p))
  in
  loop
...
let mark_ancestors base scanned treshold =
  let rec loop p =
    let i = get_iper p in
    if not @@ Gwdb.Marker.get scanned i then (
      Gwdb.Marker.set scanned i true;
      (match Gwaccess_util.oldest_year_of p with
...
let public_all ~mem bname treshold =
  ...
  Consang.check_noloop base (function
    | OwnAncestor p ->
...
let speclist =
  [
    ( "-y",
      Arg.Int (fun i -> treshold := i),
      "treshold year. Anybody born before this year is considered old (default \
       = " ^ string_of_int !treshold ^ ")" );
    ("-everybody", Arg.Set everybody, "set flag public to everybody.");
    ( "-ind",
      Arg.String (fun x -> ind := x),
      "individual key. Process only this individual and its ancestors." );
...
let () =
  ...
  Lock.control_retry (Files.lock_file !bname) ~onerror:Lock.print_error_and_exit
  @@ fun () ->
  if !everybody then
    if !ind <> "" then
      failwith "-everybody and -ind options are mutually exclusive"
    else if !treshold <> 1900 then
      failwith "-everybody and -y options are mutually exclusive"
