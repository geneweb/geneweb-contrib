...
let default_lang =
  try Sys.getenv "LC_ALL"
  with Not_found -> (
    try Sys.getenv "LC_MESSAGES"
    with Not_found -> ( try Sys.getenv "LANG" with Not_found -> "en"))
...
let exec_wait conf prog args =
  let wnd =
    GWindow.window
      ...
      ~position:`CENTER ~resizable:true ~width:600 ~height:300 ()
  in
  ...
  let redirect channel =
    ...
    channel_redirector channel (fun c ->
        buffer#insert (Bytes.to_string c);
        true)
  in
...
let close_server conf =
  ...
  | Some server_pid ->
      ...
      (try Unix.connect s (Unix.ADDR_INET (Unix.inet_addr_loopback, conf.port))
...
let select_dir parent initial_dir =
  ...
  let new_dir =
    if dialog#run () = `OPEN then
      match dialog#filename with Some dir -> dir | _ -> initial_dir
    else initial_dir
  in
  dialog#destroy ();
  new_dir
...
let select_file parent initial_file =
  ...
  let new_file =
    if dialog#run () = `OPEN then
      match dialog#filename with Some file -> file | _ -> initial_file
    else initial_file
  in
  dialog#destroy ();
  new_file
...
let delete_base conf bname =
  let wnd =
    GWindow.window
      ...
      ~position:`CENTER ~resizable:true ~width:300 ~height:50 ()
  in
  let vbox = GPack.vbox ~spacing:5 ~packing:wnd#add () in
  let bbox =
    GPack.button_box `HORIZONTAL ~border_width:5 ~layout:`SPREAD
...
let create_base conf bname src_file =
  if bname = "" then ()
    ...
    if Sys.file_exists gwf_file then () else print_default_gwf_file conf bname)

let rename_base conf bname new_name =
  let databases =
    List.sort compare
      (List.filter
         (fun fn -> Filename.check_suffix fn ".gwb")
...
let merge conf bnames bname parameters =
  ...
  List.iter
    (fun bname ->
      ...
      let c = Filename.concat bin_dir "gwu " ^ bname ^ " -o " ^ bname ^ ".gw" in
      let rc = Sys.command c in
      if rc > 1 then error_popup c)
    bnames;
  let old_bases =
    List.fold_left (fun accu bname -> accu ^ " -sep " ^ bname ^ ".gw") "" bnames
  in
...
let main_window =
  ...
  let wnd =
    GWindow.window
      ...
      ~position:`CENTER ~resizable:true ~width:640 ~height:480 ()
  in
...
let rec show_main conf =
  ignore
    (main_window#connect#destroy ~callback:(fun () ->
         close_server conf;
         GMain.quit ()));
  main_window#show ();
  clean_waiting_pids conf;
  ...
  let inser_toolbar text tooltip icon_file callback =
    toolbar#insert_button ~text ~tooltip ~tooltip_private:"Private"
      ~icon:(icon icon_file) ~callback ()
  in
  ignore
    (inser_toolbar
       (capitale (transl "create"))
       (capitale (transl "create a database"))
       "gui_create.png"
       (fun () ->
         vbox#destroy ();
         new_database conf));
  ...
  *)
  ignore
    (inser_toolbar
       (capitale (transl "quit"))
       (capitale (transl "quit GeneWeb"))
       "gui_quit.png"
       (fun () ->
         vbox#destroy ();
         close_server conf;
  ...
  if databases = [] then
    ...
    let _label =
      GMisc.label
        ~text:
          (capitale (transl "available databases")
          ^ " ("
...
and new_database conf =
  ...
  let _label =
    GMisc.label
      ...
      ()
  in
  let entry = GEdit.entry ~text:"" ~packing:(table#attach ~left:1 ~top:0) () in
  let _label =
    GMisc.label
  ...
  let file = ref "" in
  ignore (select#connect#clicked (fun () -> file := select_file main_window ""));
  let bbox =
    GPack.button_box `HORIZONTAL ~border_width:5 ~layout:`SPREAD
      ~packing:vbox#pack ()
  in
  ...
  in
  ignore
    (btn_cancel#connect#clicked (fun () ->
         vbox#destroy ();
         show_main conf));
  let btn_ok = GButton.button ~label:(transl "OK") ~packing:bbox#pack () in
  ignore
    (btn_ok#connect#clicked (fun () ->
         create_base conf entry#text !file;
         vbox#destroy ();
         show_main conf))
...
and setup_gui conf =
  ...
  in
  ignore
    (select#connect#clicked (fun () ->
         conf.bases_dir <- select_dir main_window conf.bases_dir));
  let hbox = GPack.hbox ~spacing:5 ~packing:vbox#pack () in
  let _label =
  ...
  let browser =
    match conf.browser with Some browser -> browser | None -> ""
  in
  let hbox = GPack.hbox ~spacing:5 ~packing:vbox#pack () in
  let _label =
  ...
  ignore
    (btn_ok#connect#clicked (fun () ->
         vbox#destroy ();
         let browser = match conf.browser with Some b -> b | None -> "" in
         let gui_arg =
           [
             ("bd", conf.bases_dir);
             ("port", string_of_int conf.port);
             ("browser", browser);
           ]
         in
         let conf = { conf with gui_arg } in
...
and tools conf bname =
  ...
  let _label =
    GMisc.label ~text:(transl "Rename")
  ...
  ignore
    (bbut#connect#clicked (fun () ->
         ...
         show_main conf));
  let _label =
    GMisc.label ~text:(transl "Delete")
...
let launch_config () =
  if Sys.file_exists config_gui_file then
    ...
    | Some b ->
        ...
        in
        ignore
          (btn#connect#toggled ~callback:(fun () ->
               browser := b;
               let num = assistant#current_page in
               let page = assistant#nth_page num in
    ...
    ignore
      (assistant#append_page ~title:(transl "Setup browser") ~page_type:`CONTENT
         page_3#as_widget);
    ignore
      (assistant#append_page ~title:(transl "Completed") ~page_type:`CONFIRM
      ...
      let conf =
        {
          ...
          log = Filename.concat !bases_dir "comm.log";
          gui_arg;
          gwd_arg = [];
          only_arg = [];
          server_running = None;
