...
let nameize _ =
  ...
  let len =
    if two then len
      ...
      let len = Buff.mstore len (char2 ()) in
      len
  in
     ...
     if n > ngen then ()
       ...
       for i = 0 to size - 1 do
         if List.mem i celib then ()
           ...
           in
           let list =
             aux [] Male h h_fn (if h_before_f then 0 else 2 * 365) true
           in
           let list =
             aux list Female f f_fn
               (if h_before_f then 2 * 365 else 0)
               (not h_before_f)
           in
...
let speclist =
  [
    ("-f", Arg.Set force, " force database creation (erase existing base)");
    ( "-g",
      Arg.Int (fun i -> gyear := i),
      "AGE age at marriage (" ^ string_of_int !gyear ^ ")" );
    ( "-n",
      Arg.Int (fun i -> ngen := i),
