...
let number_of_desc base mark p =
  ...
  let rec loop nb_list nb new_gen = function
    | ifam :: ifaml ->
        let nb, new_gen =
...
let nb_desc bname date =
  ...
  Printf.printf "nombre de personnes vivantes en %s : %d\n"
    (string_of_date date) nb_liv;
  flush stdout;
  let mark = Gwdb.ifam_marker (Gwdb.ifams base) 0 in
  let nb_desc = ref [] in
  ProgrBar.start ();
  ignore
    (apply base date (fun cnt _ip p ->
...
let date year = { day = 0; month = 0; year; prec = Sure; delta = 0 }
