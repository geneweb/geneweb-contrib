...
let has_infos p = match p.death with Death (_, _) -> true | _ -> false
...
let modif base =
  ...
  for i = 0 to Gwdb.nb_of_persons base - 1 do
    ...
    if match fn with "ne" -> true | _ -> false then (
      Printf.eprintf ".";
      flush stderr;
  ...
  if !changes then (
    ...
    Printf.eprintf
      "\n\
       Attention: il n'est pas sûr que les index aient été complètement mis à \
       jour.\n\
       Pour faire bien, il faudrait vérifier si le programme le fait \
       correctement.\n\
       Dans le doute, il est probablement préférable de faire gwu et gwc \
       (nettoyage)\n\
...
let main () =
  ...
  let base = Iobase.input !bname in
  modif base
