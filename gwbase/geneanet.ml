...
let line_of_person base p =
  {
    name = sou base (get_surname p);
    info = info_of_person base p;
    dbeg = begin_of_person base p;
    dend = end_of_person base p;
    nbindi = 1;
    place = place_of_person base p;
    subregion = "";
    region = region_of_person base p;
    country = country_of_person base p;
  }
...
module Line = Map.Make (struct
  type t = line

  let compare = compare_lines
end)

let print_line line =
  if
    line.info <> "" || line.dbeg <> infinity || line.dend <> 0
  ...
  then (
    ...
    if line.dend = 0 then Printf.printf ";" else Printf.printf "%d;" line.dend;
...
let geneanet base =
  ...
  Gwdb.Collection.iter
    (fun p ->
      ...
      if Gwdb.Marker.get mark (get_iper p) && surname <> "?" && surname <> "x"
      then
    ...
    (Gwdb.persons base);
  (let list = Line.fold (fun _ line list -> line :: list) !lines [] in
