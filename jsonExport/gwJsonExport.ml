...
let print_parents base basename person =
  ...
  | Some ifam ->
      ...
      let person_id =
        Printf.sprintf "persons/%s" (key_of_person basename person)
      in
      let father_id =
        Printf.sprintf "persons/%s" (key_of_person basename father)
      in
      let mother_id =
        Printf.sprintf "persons/%s" (key_of_person basename mother)
      in
      let json_father =
        `Assoc
          [
            ("_from", `String father_id);
            ("_to", `String person_id);
            ("basename", `String basename);
          ]
      in
      let json_mother =
        `Assoc
          [
            ("_from", `String mother_id);
            ("_to", `String person_id);
            ("basename", `String basename);
          ]
      in
      Printf.printf "%s\n%s\n"
        (Yojson.Basic.to_string json_father)
        (Yojson.Basic.to_string json_mother)

let print_family base basename family =
  let key =
    Printf.sprintf "%s:%s" basename (string_of_ifam @@ get_ifam family)
  in
...
let print_json basename mode =
  ...
  match mode with
  | "persons" ->
      Gwdb.Collection.iter (print_person base basename) (Gwdb.persons base)
  | "parents" ->
      Gwdb.Collection.iter (print_parents base basename) (Gwdb.persons base)
  | "families" ->
      Gwdb.Collection.iter (print_family base basename) (Gwdb.families base)
  | _ -> ()
...
let speclist =
  [
    ( "-mode",
      Arg.String set_mode,
      "the mode to work in : persons|parents|families" );
  ]
