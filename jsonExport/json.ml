...
let rec json_list_of_istr_list base strlist json =
  ...
  | [] -> json
  | str :: remains ->
      let json = json @ [ json_of_istr base str ] in
      json_list_of_istr_list base remains json
...
let json_of_dmy dmy =
  `Assoc
    [
      ("day", `Int dmy.day); ("month", `Int dmy.month); ("year", `Int dmy.year);
    ]
...
let rec json_of_pevents base pevents json =
  ...
  | [] -> json
  | pevent :: remains ->
      let json = [ json_of_pevent base pevent ] @ json in
      json_of_pevents base remains json

let json_of_title_name base name =
  match name with
  | Tmain -> `String ""
  | Tname s -> json_of_istr base s
  | Tnone -> `Null

let json_of_title base gen_title =
  `Assoc
    [
...
let rec json_of_titles base gen_titles json =
  ...
  | [] -> json
  | gen_title :: remains ->
      let json = [ json_of_title base gen_title ] @ json in
      json_of_titles base remains json

let json_of_relation_type r_type =
  match r_type with
...
let json_of_iper_option iper_opt =
  match iper_opt with Some p -> `String (string_of_iper p) | None -> `Null

let json_of_rparent base gen_relation =
  `Assoc
    [
...
let rec json_of_rparents base gen_relations json =
  ...
  | [] -> json
  | gen_relation :: remains ->
      let json = [ json_of_rparent base gen_relation ] @ json in
      json_of_rparents base remains json

let rec json_of_ipers_list related json =
  match related with
  | [] -> json
  | rel :: remains ->
      let json = [ `String (string_of_iper rel) ] @ json in
      json_of_ipers_list remains json
...
let json_of_person base person =
  ...
  let key =
    make_key
      (sou base (get_surname person))
      (sou base (get_first_name person))
      occ
  in
  let index = string_of_iper (get_iper person) in
  let access =
    match get_access person with Private -> 2 | Public -> 1 | _ -> 0
  in
  let sex = match get_sex person with Male -> 1 | Female -> 2 | _ -> 0 in
  ...
  let first_names_aliases =
    json_list_of_istr_list base (get_first_names_aliases person) []
  in
  let surnames_aliases =
    json_list_of_istr_list base (get_surnames_aliases person) []
  in
...
let json_of_relation_kind relation_kind =
  match relation_kind with
...
let json_of_divorce divorce =
  match divorce with
  | NotDivorced -> `Null
  | Divorced date -> json_of_cdate date
  | Separated -> `Bool true

let json_of_fevent_name base name =
  match name with
...
let json_of_fevent_witness witness =
  match witness with
  | person, witness_kind ->
      `Assoc
        [
          ("person", `String (string_of_iper person));
...
let rec json_of_fevent_witnesses witnesses json =
  ...
  | [] -> json
  | witness :: remains ->
      let json = [ json_of_fevent_witness witness ] @ json in
      json_of_fevent_witnesses remains json

let json_of_fevent base fevent =
  `Assoc
    [
      ...
      ( "witnesses",
        `List
          (json_of_fevent_witnesses (Array.to_list fevent.efam_witnesses) []) );
    ]
...
let rec json_of_fevents base pevents json =
  ...
  | [] -> json
  | pevent :: remains ->
      let json = [ json_of_fevent base pevent ] @ json in
      json_of_fevents base remains json
...
let json_of_family base family =
  ...
  let ifam = string_of_ifam (get_ifam family) in
  ...
  let parents =
    json_of_ipers_list (Array.to_list (get_parent_array family)) []
  in
  `Assoc
    [
