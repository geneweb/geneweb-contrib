...
let suspend_with msg =
  ProgrBar.suspend ();
  msg ();
  flush stdout

let restart_with_fixed i n =
  Printf.printf "\t\tfixed\n";
  flush stdout;
...
let check_burial base ~verbosity1 ~verbosity2 nb_ind cnt =
  ...
  let fix p =
    ...
    | UnknownBurial -> (
        ...
        | None ->
            if
              (not (is_empty_string (get_burial_place p)))
  ...
  Gwdb.Collection.iteri
    (fun i p ->
      ...
      if fix p then (
        incr cnt;
        if verbosity2 then
          Printf.printf "Modifiy person : %s\n%!" (Gutil.designation base p)))
    (Gwdb.persons base);
  ...
  Gwdb.Collection.iteri
    (fun i fam ->
      ...
      for j = 0 to Array.length children - 1 do
        ...
        else if parents <> Some ifam && verbosity1 then (
                 ...
                 if verbosity2 then
                   suspend_with (fun () ->
                       Printf.printf "\tRemoving ifam %s from [%s] unions\n"
                         (string_of_ifam ifam) (string_of_p base ip));
                 incr fix;
                 acc)
               else ifam :: acc)
             ifams []
            ...
            if not (List.memq ip (get_related p2)) then (
              if verbosity2 then
                suspend_with (fun () ->
                    ...
                    Printf.printf
                      "\twitness has no pointer to persons' event: %s\n"
                      (string_of_p base ip));
            ...
            if not (List.memq ifath (get_related p)) then (
              if verbosity2 then
                suspend_with (fun () ->
                    ...
                    Printf.printf
                      "\twitness has no pointer to family event: %s\n"
                      (string_of_p base ip));
...
let check ~verbosity ~fast ~f_parents ~f_children ~p_parents ~p_families
    ~p_burial ~pevents_witnesses ~fevents_witnesses ~marriage_divorce bname =
  ...
  if !f_parents then
    check_families_parents ~verbosity1 ~verbosity2 base nb_fam fix;
  if !f_children then
    check_families_children ~verbosity1 ~verbosity2 base nb_fam fix;
  if !p_parents then
    check_persons_parents ~verbosity1 ~verbosity2 base nb_ind fix;
  if !p_burial then check_burial base ~verbosity1 ~verbosity2 nb_ind fix;
  if !p_families then
    check_persons_families ~verbosity1 ~verbosity2 base nb_ind fix;
  if !pevents_witnesses then
    check_pevents_witnesses ~verbosity1 ~verbosity2 base nb_ind fix;
  if !fevents_witnesses then
    check_fevents_witnesses ~verbosity1 ~verbosity2 base nb_fam fix;
  if !marriage_divorce then
    fix_marriage_divorce ~verbosity1 ~verbosity2 base nb_fam fix;
  if fast then (
...
let main () =
  ...
  if
    ...
    || !fevents_witnesses || !marriage_divorce || !p_burial
  then ()
  else (
    f_parents := true;
    f_children := true;
    ...
    p_burial := true);
  check ~fast ~verbosity ~f_parents ~f_children ~p_burial ~p_parents ~p_families
    ~pevents_witnesses ~fevents_witnesses ~marriage_divorce !bname
