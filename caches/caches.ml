...
let add istr_set istr =
  if not (Gwdb.is_empty_string istr) then
    IstrSet.add istr istr_set
  else
    istr_set

let add_lastname cache istr =
  {cache with lastname = add cache.lastname istr}

let add_first_name cache istr =
  {cache with first_name = add cache.first_name istr}

let add_source cache istr =
  {cache with source = add cache.source istr}

let add_occupation cache istr =
  {cache with occupation = add cache.occupation istr}

let add_place cache istr =
  {cache with place = add cache.place istr}
...
let create_cache_data base =
  ...
  let cache = Gwdb.Collection.fold add_person_infos_to_cache empty_cache persons in
  let families = Gwdb.families base in
  Gwdb.Collection.fold add_family_infos_to_cache cache families

let lastname_cache_fname base_file =
  Filename.concat base_file "cache_surname"

let first_name_cache_fname base_file =
  Filename.concat base_file "cache_first_name"

let place_cache_fname base_file =
  Filename.concat base_file "cache_place"

let source_cache_fname base_file =
  Filename.concat base_file "cache_src"
